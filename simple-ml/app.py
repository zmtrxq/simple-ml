...
# Converts a matplotlib figure to base64 string for embedding in HTML
...
# Generates a heatmap plot showing correlations between numeric features
...
# Creates a horizontal bar plot showing feature importances for tree-based models
...
# Constructs a preprocessing pipeline for numeric and categorical features
...
# Retrieves feature names after transformation, with fallback if not supported
...
# Handles uploaded training CSV and returns column names and preview HTML
...
# Main route for processing training and testing data and returning model results
@app.route('/process', methods=['POST'])
def process_data_route():
    try:
        train_file = request.files.get('train_file'); test_file = request.files.get('test_file')
        target_column_name_from_user = request.form.get('target_column')
        ...
        original_train_feature_cols = X_train_raw.columns.tolist()
        for col in original_train_feature_cols:
            if col not in X_test_raw.columns: X_test_raw[col] = 0
        ...
        for model_name, model_instance in models_def[problem_type].items():
            ...
            if summary_row and len(summary_row) > 1 : summary_metrics_data.append(summary_row)
            ...
            test_preds_final = test_preds_numeric
            ...
            results_output.append({'model': model_name, 'metrics': model_detailed_cv_metrics,
                                   ...
                                   'test_set_predictions': test_preds_final.tolist()
                                  })
        ...
        return jsonify({'results': results_output, 'correlation_plot': correlation_plot_b64,
                        ...
                        'target_column_name': target_column_name_from_user
                       })
    except Exception as e:
