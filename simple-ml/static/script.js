document.addEventListener('DOMContentLoaded', () => {
    // --- (All const declarations for UI elements - same as previous full script.js) ---
    ...
    let globalResultsData = null; // To store results for download

    // --- (updateUIState, fetchAndDisplayColumns, populateColumnSelectors - same as previous full script.js) ---
    ...
    trainFileInput.addEventListener('change', async () => {
        if (trainFileInput.files.length > 0) {
            ...
            columnSelectorContainer.innerHTML = ''; dfHeadPreviewContainer.innerHTML = '';
            updateUIState(0); 
        }
    });
    ...
    function populateColumnSelectors(columns) {
        ...
        if (columns.length === 0) { columnSelectorContainer.innerHTML = '<p>No columns found.</p>'; return; }
        ...
        columns.forEach((colName, index) => {
            ...
            const cellName = row.insertCell(); cellName.textContent = colName; cellName.classList.add('col-name-cell');
            const cellTarget = row.insertCell(); cellTarget.classList.add('select-cell');
            const targetRadio = document.createElement('input'); targetRadio.type = 'radio';
            targetRadio.name = 'target_column_radio'; targetRadio.value = colName; targetRadio.id = `target_col_${index}`;
            if ((index === columns.length - 1 && columns.length > 1) || columns.length === 1) targetRadio.checked = true;
            cellTarget.addEventListener('click', () => targetRadio.click()); cellTarget.appendChild(targetRadio);
            const cellDrop = row.insertCell(); cellDrop.classList.add('select-cell');
            const dropCheckbox = document.createElement('input'); dropCheckbox.type = 'checkbox';
            dropCheckbox.value = colName; dropCheckbox.id = `drop_col_${index}`; dropCheckbox.classList.add('drop-column-checkbox');
            cellDrop.addEventListener('click', (e) => { if (e.target !== dropCheckbox) dropCheckbox.checked = !dropCheckbox.checked; });
    ...
    runModelsButton.addEventListener('click', async () => {
        const trainFile = trainFileInput.files[0]; const testFile = testFileInput.files[0];
        ...
        const formData = new FormData();
        formData.append('train_file', trainFile); formData.append('test_file', testFile);
        formData.append('target_column', targetColumn); formData.append('columns_to_drop', JSON.stringify(columnsToDrop));
        formData.append('problem_type', problemType);

        resultsArea.innerHTML = ''; generalPlotsArea.innerHTML = ''; summaryTableArea.innerHTML = '';
        if(generalDivider) generalDivider.style.display = 'none';
        if(summaryDivider) summaryDivider.style.display = 'none';
        loadingIndicator.style.display = 'block'; runModelsButton.disabled = true;
        globalResultsData = null; // Reset global results
        ...
        try {
            ...
            if (response.ok) {
                if (data.error) { 
                    ...
                    globalResultsData = data; // Store all data for potential downloads
                    displaySummaryTable(data.summary_metrics, data.problem_type);
                    displayGeneralPlots(data.correlation_plot); 
                    displayModelResults(data.results, data.problem_type, data.target_column_name); // Pass target_column_name
                    if(data.summary_metrics && data.summary_metrics.length > 0) {
                        if(summaryDivider) summaryDivider.style.display = 'block';
                    }
                    if(data.correlation_plot || (data.results && data.results.length > 0)) {
                        if(generalDivider) generalDivider.style.display = 'block';
                    }
                }
            } else {
                let errorMsg = `Server responded with status ${response.status}.`;
                if (data && data.error) { errorMsg += ` Message: ${data.error}`; } // Use error from JSON if available
                resultsArea.innerHTML = `<p class="warning-text">Error: ${errorMsg}</p>`;
            }
        } catch (error) { // Network errors or non-JSON responses
            resultsArea.innerHTML = `<p class="warning-text">Network Error processing data. (${error.message})</p>`;
        } finally {
            loadingIndicator.style.display = 'none'; runModelsButton.disabled = false;
        }
    });

    // --- (displaySummaryTable, displayGeneralPlots - same as previous full script.js) ---
    function displaySummaryTable(summaryData, problemType) {
        ...
        summaryData.forEach(row => {
            ...
            headers.forEach(header => {
                let cellValue = row[header]; let cellClass = '';
                if (header.toLowerCase() === 'accuracy' && problemType === 'classification') { 
                    cellClass = 'metric-accuracy'; cellValue = (parseFloat(cellValue) * 100).toFixed(2) + '%';
                } else if (typeof cellValue === 'number') { cellValue = cellValue.toFixed(4); }
    ...
    // --- MODIFIED displayModelResults ---
    // Displays detailed results and plots for each individual model, now with download button.
    function displayModelResults(results, problemType, targetColumnName) { // Added targetColumnName
        ...
        results.forEach((result, index) => { // Added index for unique button IDs
        ...
        if (html.endsWith('<hr class="divider">')) { html = html.substring(0, html.lastIndexOf('<hr class="divider">')); } 
        ...
        document.querySelectorAll('.download-preds-button').forEach(button => {
            button.addEventListener('click', (event) => {
                ...
                if (globalResultsData && globalResultsData.results && globalResultsData.results[modelIndex]) {
    ...
    function downloadPredictionsCSV(predictions, modelName, targetHeader) {
        ...
        // Use the original target column name passed from backend, or default to "target"
        const header = targetHeader; 
        let csvContent = header + "\n"; // CSV Header
        predictions.forEach(pred => {
            // Handle potential commas or newlines in predictions if they are strings
            let formattedPred = String(pred);
            if (formattedPred.includes(",")) {
                formattedPred = `"${formattedPred.replace(/"/g, '""')}"`; // Quote and escape existing quotes
            }
